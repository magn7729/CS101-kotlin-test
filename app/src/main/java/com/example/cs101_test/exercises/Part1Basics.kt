...
object Part1Basics {
    ...
    fun secondLargest(list: List<Int>): Int {
        val sortedList = list.sortedDescending()[1]
        return sortedList
    }
    ...
    fun charFrequency(str: String): Map<Char, Int> {
        ...
        for (char in str) {
            if (char.isLetter()) {
                ...
                if (frequencyMap[lowerChar] == null) {
                    frequencyMap[lowerChar] = 1
                } else {
                    frequencyMap[lowerChar] = frequencyMap[lowerChar]!! + 1
                }
            }
        }
        return frequencyMap
...
// Har fået hjælp af ChatGPT til struktur i denne opgave